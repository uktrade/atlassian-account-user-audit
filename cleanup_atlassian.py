...
def cleanup(base_url=ATLASSIAN_URL,organisation_name=ATLASSIAN_ORG_NAME,api_key=ATLASSIAN_AUTH_TOKEN,reason=REASON,last_active=THREE_MONTHS_AGO):
    """
    ...
    last_active = (datetime.strptime(last_active,'%d-%m-%Y')).replace(tzinfo=UTC).isoformat()
    ...
    if orgId:
        ...
        for user in not_active_users:
            msg = {'message': {reason}}
            print(f"cleaning up: {user['name']} because their last access was: {user['last_active']}")
            atlassian_client.disable_user(user['account_id'], body=msg)

    else:
        raise RuntimeError(f'Error: Atlassian organisation "{organisation_name}" not found')
